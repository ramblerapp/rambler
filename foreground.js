...
function scbt_get_chat_messages() {
  var divs = document.body.querySelectorAll('[data-chat-entry]');
  return divs;
}
...
function scbt_get_clean_username(obj) {
  if (obj && obj.username) {
    ...
    if ( ( obj.username.indexOf('bot') > -1 ) || ( obj.username == 'streamelements') || ( obj.username == 'streamlabs') ) {
      obj.isBot = 1;
    }
  }
  return obj;
...
// TODO FOR VODS
function scbt_get_all_chat_print(db) {
  if (!db || !db.objectStoreNames.length || db.objectStoreNames.length < 1) { return false; }
    var chats = x = option = error = error2 = request = obj = transaction = store = x = theHTML = newElement = null; 
    ...
    store.getAll().onsuccess = function(e) {
      console.log(e);
      if (!e) { setTimeout(function(){ scbt_helper_toast('get scbt_get_all_chat_print failed.'); }, 2700); var chats = x = option = e = e2 = error = error2 = request = obj = db = transaction = store = x = theHTML = newElement = null;  return false; }
      if (!e.target) { setTimeout(function(){ scbt_helper_toast('get scbt_get_all_chat_print target failed.'); }, 2700); var chats = x = option = e = e2 = error = error2 = request = obj = db = transaction = store = x = theHTML = newElement = null;  return false; }
      if (!e.target.result) { setTimeout(function(){ scbt_helper_toast('get scbt_get_all_chat_print target result failed.'); }, 2700); var chats = x = option = e = e2 = error = error2 = request = obj = db = transaction = store = x = theHTML = newElement = null;  return false; }

      var chats = e.target.result;
      if (chats.length < 1) { setTimeout(function(){ scbt_helper_toast('this stream chat not found for display'); }, 2700); var chats = x = option = e = e2 = error = error2 = request = obj = db = transaction = store = x = theHTML = newElement = null; return false; }
      var x = document.body.querySelectorAll('aside')[0];
      var theHTML = "<section class='chat'><div class='chat--container container'><div class='chat--header'><h1>Previous Chat</h1><div class='chat--height'><div class='chat-history'><ul id='chat-history-list'><li></li></ul></div></div></div></div></section>";
      if (x) { x.insertAdjacentHTML('afterbegin', theHTML ); }
      chats = scbt_get_sort_times(chats);
      var x = document.getElementById('chat-history-list');
      [].forEach.call(chats, function(item) {
          ...
          var newElement = '<li class="chat-history--row"><div class="chat-history--message-wrapper"><div class="chat-history--username"><a target="_blank" href="#" style="color: #15a88d">' + item.username + '</a></div><div class="chat-history--message">' + item.message + '</div></div></li>';
          x.insertAdjacentHTML('beforeend', newElement);
      });
      scbt_helper_insert_name_into_chat();
      var chats = x = option = e = e2 = error = error2 = request = obj = db = transaction = store = x = theHTML = newElement = null; return false;
    }; // objectStore.getAll
    store.getAll().onerror = function(error) {
      scbt_handle_db_error_blocked(error);
      var chats = x = option = e = e2 = error = error2 = request = obj = db = transaction = store = x = theHTML = newElement = null; return false;
    };
    store.getAll().onblocked = function(error) {
      scbt_handle_db_error_blocked(error);
      var chats = x = option = e = e2 = error = error2 = request = obj = db = transaction = store = x = theHTML = newElement = null; return false;
    };
}
...
function scbt_get_role_from_message(div, parameter) {
  var toReturn = 0;

  if (parameter == 'sub') {
    if ( 
      (div.querySelectorAll('img[alt="Subscriber"]').length > 0) || 
      (div.querySelectorAll("svg g#surface1 path[style='stroke:none;fill-rule:nonzero;fill:rgb(0.784314%,27.843137%,11.372549%);fill-opacity:1;']").length > 0 )
    ) {
      toReturn = 1;
      return toReturn;  
    }
  ...
  if (parameter == 'moderator') {
    if ( div.querySelectorAll("svg g#surface1 path[style='stroke:none;fill-rule:nonzero;fill:rgb(40%,78.431373%,100%);fill-opacity:1;']").length > 0 ) {
      toReturn = 1;
      return toReturn;
    }
  }
  ...
  if (parameter == 'founder') {
    if ( div.querySelectorAll("svg g#surface1 path[style='stroke:none;fill-rule:nonzero;fill:rgb(100%,68.235294%,0%);fill-opacity:1;']").length > 0 ) {
      toReturn = 1;
      return toReturn;
    }
  }
  ...
  if (parameter == 'verified') {
    if ( ( div.querySelectorAll("svg g#surface1 path[style='stroke:none;fill-rule:nonzero;fill:rgb(22.745098%,82.745098%,1.960784%);fill-opacity:1;']").length > 0 ) || ( div.querySelectorAll("svg path[fill='#00d0ff']").length > 0 ) ) {
      toReturn = 1;
      return toReturn;
    }
  }
  ...
  if (parameter == 'og') {
    if ( div.querySelectorAll("svg g#surface1 path[style='stroke:none;fill-rule:nonzero;fill:rgb(100%,27.058824%,27.058824%);fill-opacity:1;']").length > 0 ) {
      toReturn = 1;
      return toReturn;
    }
  }
  ...
  if (parameter == 'owner') {
    if ( div.querySelectorAll("svg g#surface1 path[style='stroke:none;fill-rule:nonzero;fill:rgb(63.921569%,30.196078%,100%);fill-opacity:1;']").length > 0 ) {
      toReturn = 1;
      return toReturn;
    }
  }
  ...
  if (parameter == 'staff') {
    if ( div.querySelectorAll("svg g#surface1 path[d='M 8 14 L 5 11 L 7 5 L 9 5 L 11 11 Z M 8 14 ']").length > 0 ) {
      toReturn = 1;
      return toReturn;
    }
  }
  ...
  if (parameter == 'gifter') {
    if ( 
       (div.querySelectorAll("g[clip-path='url(#clip0_301_17830)']") .length > 0)
    || (div.querySelectorAll("g[clip-path='url(#clip0_301_17825)']") .length > 0)
    || (div.querySelectorAll("g[clip-path='url(#clip0_301_17820)']") .length > 0)
    || (div.querySelectorAll("g[clip-path='url(#clip0_301_17815)']") .length > 0)
    || (div.querySelectorAll("g[clip-path='url(#clip0_301_17810)']") .length > 0)
    || (div.querySelectorAll("g[clip-path='url(#clip0_301_17805)']") .length > 0)
    || (div.querySelectorAll("g[clip-path='url(#clip0_301_17800)']") .length > 0)
    ) {
      toReturn = 1;
      return toReturn;  
    }
  ...
  div = null; parameter = null; return toReturn;
}
...
function scbt_helper_build_all_menus(a,b,c) {
  ...
  if (x) { } else { 
    ...
    scbt_helper_toast( window.scbtService + ' KickStarr loaded, RUNNING version: ' + chrome.runtime.getManifest().version);
...
function scbt_helper_chat_auto_show() {
  if (window.scbtOptions.scbtfeature5 === true) {
    ...
    var x = document.querySelectorAll('#main-view .channel.relative.h-full.w-full.overflow-hidden .relative.flex.h-full.w-full > div:last-of-type');
    if (x[1]) {
...
function scbt_helper_chat_clean(obj, div) {
  ...
  // HOST EVENT
  var h = div.querySelector('.chatroom-event-host__container');
  if (h) {
    special = true;
    var n = div.querySelector('.chatroom-event-host__label .text-white');
    obj.username = null;
    if (n) {
    ...
    var n = div.querySelector('.chatroom-event-host__label');
    obj.message = null;
    if (n) {
      var name =  n.innerText;
      if (name) {
  ...
  // SUB EVENT
  var h = div.querySelector('.chatroom-event-sub__container');
  if (h) {
    special = true;
    obj.username = 'anonymous';
    obj.message = 'just subscribed for 1 month';

    var n = div.querySelector('.chatroom-event-sub__label .text-white');
    if (n) {
      var name =  n.innerText;
      if (name) {
        obj.username = name;
    ...
    var n = div.querySelector('.chatroom-event-sub__label');
    if (n) {
      var name =  n.innerText;
      if (name) {
        obj.message = name;
  ...
  // GIFT EVENT
  var h = div.querySelector('.chatroom-event-subgift__container');
  if (h) {
    special = true;
    obj.username = 'anonymous';
    obj.message = 'has gifted 1 subscription to the community';

    var n = div.querySelector('.chatroom-event-subgift__label .text-white');
    if (n) {
      var name =  n.innerText;
      if (name) {
        obj.username = name;
    ...
    var n = div.querySelector('.chatroom-event-subgift__label');
    if (n) {
      var name =  n.innerText;
      if (name) {
        obj.message = name;
  ...
  // CLIP EVENT
  var h = div.querySelector('.chat-entry-clip');
  if (h) {
    special = true;
    obj.username = 'anonymous';
    obj.message = 'made a nice clip.';
    var n = h.querySelector('.text-primary');
    if (n) {
      var name =  n.innerText;
      if (name) {
        obj.username = name;
    ...
    var m = h.querySelector('.chat-entry-clip-info');
    if (m) {
      var name =  m.innerText;
      if (name) {
        obj.message = name;
      }
    }
    var i = h.querySelector('img');
    if (i) {
      obj.message = obj.message + ' img ' + i.src;
    }
    obj.timestamp = '1234567';
  }


  if (special === false) {
    var timestamp = div.querySelector('.inline-block.text-xs.font-semibold.text-gray-400');
    if (timestamp) {
      obj.timestamp = timestamp.innerText;
    }

    var n = div.querySelector('.chat-entry-username');
    if (n) { } else {
      var n = div.querySelector('.chat-message-identity');
    }
    ...
    var m = div.querySelector('.chat-entry-content');
    obj.message = null;
    if (m) {
      var name = m.innerText;
      if (name) {
    ...
    if (div.classList.contains('kicks') ) {
      obj.donation = 1;
    }

    obj.sub = scbt_get_role_from_message(div, 'sub'); // 0 or 1
    obj.moderator = scbt_get_role_from_message(div, 'moderator');
    obj.founder = scbt_get_role_from_message(div, 'founder');
    obj.verified = scbt_get_role_from_message(div, 'verified');
    obj.og = scbt_get_role_from_message(div, 'og');
    obj.owner = scbt_get_role_from_message(div, 'owner');
    obj.gifter = scbt_get_role_from_message(div, 'gifter');
    obj.staff = scbt_get_role_from_message(div, 'staff');
  }
  ...
  if (obj.username && obj.message) {
    if (special === true) {
      ...
      if (div.dataset.chatEntry) {
        obj.itemid = div.dataset.chatEntry;
      } else {
        var um = obj.message;
  ...
  var special = h = n = name = timestamp = div = null; return obj;
} // end scbt_helper_chat_clean


function scbt_helper_chat_filter_blocked_links(obj, div) {
  var messageDiv = div.querySelector('.chat-entry-content');
  if (messageDiv) {
...
function scbt_helper_chat_filter_highlighted_words(obj, div) {
  ...
  var messageDiv = div.querySelector('.chat-entry .chat-entry-content');
  if (messageDiv) {
      var t = obj.message;
      if (t) {
        tm = t.toLowerCase();
        ...
        for (var i = 0; i < arrl; i++) {
          ...
          y = y.trim();
          if (tm.indexOf(y) > -1 ) {
            messageDiv.style.setProperty('font-size', '3rem', 'important');
            messageDiv.style.setProperty('line-height', '1', 'important');  
          }
        }
      }
...
function scbt_helper_chat_filter_vip_words(obj, div) {
  ...
  var messageDiv = div.querySelector('.chat-entry-username');
  if (messageDiv) {
    var t = obj.username;
    if (t) {
      tm = t.toLowerCase();
      ...
      for (var i = 0; i < arrl; i++) {
        ...
        if (tm == y) {
          ...
          var x = div.querySelector('.chat-entry .chat-entry-content');
...
function scbt_helper_chat_listen() {
  ...
  targetNode = document.body.querySelector('#chatroom .overflow-y-scroll.py-3');
  if (targetNode) {
  ...
  if (typeof obj == 'object') {
    ...
    if (obj.a == 'scbttheme1') {
      if (obj.b == '#ffffff' || obj.b == '') {
        css = css + '#chatroom { background-color: initial !important; } ';
      }
      if (obj.b == '#000000') {
        css = css + '#chatroom { background-color: transparent !important; } ';
      }   
      if ( (obj.b != '#ffffff') && (obj.b != '#000000') ) {
        css = css + '#chatroom { background-color:' + obj.b + ' !important; } '; 
      }
    }
    ...
    if (obj.a == 'scbttheme2') {
      if (obj.b == '#ffffff') {
        css = css + '.chat-entry-username { color: unset !important; } ';
      }
      if ( (obj.b != '#ffffff') && (obj.b != '#000000') ) {
        css = css + '.chat-entry-username { color:' + obj.b + ' !important; } ';
      }
    }
    ...
    if (obj.a == 'scbttheme4') {
      if (obj.b == '#ffffff') {
        css = css + '.chat-entry-content { color: unset !important; text-shadow: 0px; } ';
      }
      if (obj.b == '#000000') {
        css = css + '.chat-entry-content { color: #111111 !important; text-shadow: 0.5px 0.5px silver; } ';
      }  
      if ( (obj.b != '#ffffff') && (obj.b != '#000000') ) {
        css = css + '.chat-entry-content { color:' + obj.b + ' !important; text-shadow: 0.5px 0.5px darkslategrey; } ';
      }
    }
    ...
    if (obj.a == 'scbttheme5') {
      if (obj.b > 0) {
        var x = obj.b + 'rem';
        css = css + ' .chat-entry-content { font-size: ' + x + '; line-height: ' + obj.b  + '; } ';
      } else {
        css = css + ' .chat-entry-content { font-size: initial; line-height: unset; } ';
      }
    }
    ...
    if (obj.a == 'scbttheme6') {
      if (obj.b === true) {
        css = css + ' .chat-entry img, .chat-entry svg, .chat-entry i { visibility: hidden !important; } ';
      } else {
        css = css + ' .chat-entry img, .chat-entry svg, .chat-entry i { visibility: visible !important; } ';
      }      
    }
    ...
    if (obj.a == 'scbtfeature7') {
      if (obj.b === true) {
          css = css + ' #main-view .relative.flex.h-full.w-full { flex-direction: row-reverse; } ';
      } else {
          if (window.scbtIsMobile === true) { 
            css = css + ' #main-view .relative.flex.h-full.w-full { flex-direction: initial; } ';
          } else {
            css = css + ' #main-view .relative.flex.h-full.w-full { flex-direction: initial; } ';
          }
      }
    ...
    if (obj.a == 'scbtfeature8') {
      if (obj.b === true) {
          css = css + ' #chatroom .overflow-y-scroll.py-3 { flex-direction: column-reverse; display: flex; } ';
      } else {
          css = css + ' #chatroom .overflow-y-scroll.py-3 { flex-direction: initial; display: block; } ';
      }
    }
    ...
    if (obj.a == 'scbtfeature9') {
      if (obj.b === true) {
        css = css + ' .chat-entry-content:hover { font-size: 200% !important; } ';
       } else {
        css = css + ' .chat-entry-content:hover { font-size: initial !important; } ';
      }
    }
...
function scbt_helper_turn_on_press_chat_to_mention_menu() {
    var chats = document.querySelectorAll('.chat-entry-content');
    if (chats) {
      [].forEach.call(chats, function(chat) {
        if (chat) {
...
function scbt_helper_process_chat_line(div, saveThisChat) {
    ...
    // console.log('because we are saving? ' + window.scbtOptions.scbtfeature4 + ' ' + saveThisChat + '  going to save this chat line: ' + obj.username + ' MMM ' + obj.message + ' ' + obj.isBot + ' to DBNAME ' + window.scbtDbName);
...
function scbt_helper_load_chat_replay_from_api(videoID) {
  var streamer = 'streamer';
  var x = document.querySelector('.stream-username');
  if (x) {
    streamer = x.innerText;
  }
  var FD = new FormData();
  var obj = {};
  obj.videoID = videoID;
  ...
  if (streamer == 'Alfie' || streamer == 'Azralynn' || streamer == 'BigE' || streamer == 'DushanR' || streamer == 'Queen_Daestra' || streamer == 'maddynf' || streamer == 'meloniemac') { } else {
    return false;
  }

  FD.append( 'data', JSON.stringify( obj ) );
  fetch('https://www.streameranalytics.com/v1/api/kick/' + streamer + '/chatreplay/',
  {
    method: 'POST',
    body: FD,
    cache: 'no-cache',
  ...
  .then(function(resp){ 
  if ( (resp == null || resp.length === 0) || (isNaN(resp) == false) ) { console.log( resp ); } else {
    console.log( resp );
    if (resp == 'error') {
      setTimeout(function(){ scbt_helper_toast('Error: API Chat Request error 1'); }, 2700);
      videoID = null; x = null; FD = null; obj = null; streamer = null; resp = null; return false;
    } else {
      
      console.log(typeof resp);
      if (typeof resp === 'object') {
        
        parseInt(document.querySelectorAll('video')[0].duration % 60, 10);
        ...
        if (video.readyState > 0) {
          ...
          if (resp[1]) {
            if (resp[1][' timestamp']) {
              if (resp[1][' timestamp'].indexOf('AM') > -1) {
                startTime = resp[1][' timestamp'];
                ...
                hours = hours + 12;
                console.log("PM hours would be " + hours); // 14
        ...
        var x = document.querySelectorAll('.chat-content')[0];
        [].forEach.call(resp, function(item) {
            var diff = "0";
            var diff2 = "0";
            var timeToCompare = item[" timestamp"];
            if (timeToCompare) {
            ...
            var classString = scbt_get_classString_for_chat_replay(item);
            var newElement = '<div data-v-1833709d="" data-chat-entry="' + item[" itemid"] + '" class="break-all mt-0.5 ' + classString + '"> <div class="chat-entry"> <div> <div data-minutesin="' + diff2 + '" data-v-ff75f4fc="" class="inline-block text-xs font-semibold text-gray-400 minutesIn">' + item[" timestamp"] + '</div> <span data-v-bb151e01="" class="chat-message-identity"> <span data-v-bb151e01="" class="inline-flex translate-y-[3px] mr-1"> <div data-v-e9e28f08="" class="relative badge-tooltip ml-1 first:ml-0 ' + classString + '" active="true"> </div> </span><a target="_blank" href="https://kick.com/' + item[" username"] + '" data-v-b433ad78="" data-v-bb151e01="" class="chat-entry-username" data-chat-entry-user="z3rofax" data-chat-entry-user-id="z3rofax">' + item[" username"] + '</a></span> <span class="font-bold text-white">: </span><span data-v-89ba08de=""><span data-v-89ba08de="" class="chat-entry-content">' + item[" message"] + '</span></span> </div> </div> </div>';
            x.insertAdjacentHTML('beforeend', newElement);
        });
        videoID = null; x = null; FD = null; obj = null; streamer = null; resp = null; video = null; startTime = null; arr = null; item = null; hours = null; diff = null; diff2 = null; timeToCompare = null; classString = null; newElement = null; x = null;

        setTimeout(function() {
          var x = document.getElementsByClassName('minutesIn');
          var arrl = x.length;
          for (var i = 0; i < arrl; i++) {
            x[i].addEventListener('click', scbt_user_go_to_stream_timestamp);
          }
          x = null; arrl = null; return false;
        }, 3000);
  ...
  .catch(err => {
      console.log('connection error'); console.log(err); setTimeout(function(){ scbt_helper_toast('Error: API Chat Request error 2');}, 2700); videoID = null; x = null; FD = null; obj = null; streamer = null; resp = null; return false;
  });
...
function scbt_user_chat_down_to_bottom() {
  if (window.scbtChatWrapperRef.classList.contains('scbt-bl') ) {
    ...
    var y = document.querySelectorAll('.overflow-y-scroll.py-3');
    if (y[0]) {
      y[0].scrollTop = y[0].scrollHeight - y[0].clientHeight;
    }
    y = null; return false;
  }
}
...
function scbt_user_chat_font_size() {
  if (window.scbtChatWrapperRef.classList.contains('scbt-bl') ) {
    ...
    var divs = document.body.querySelectorAll('.chat-entry-content');
  }
...
function scbt_user_chat_full_screen_width() {
    if (window.isdoFullScreenWidth === 0) {
      var x = document.body.querySelectorAll('.main-html.h-screen.w-screen .flex.h-full.w-full > .flex.grow.overflow-hidden:first-of-type > div.relative:first-of-type');
      if (x[0]) {
  ...
  if (window.scbtChatWrapperRef.classList.contains('scbt-bl') ) {
      ...
      if (window.isdoNonBots === 0) {
          ...
          [].forEach.call(divs, function(div) {
            ...
            var u = div.querySelector('.chat-entry-username');
            if (u) {
              var un = u.innerText;
              if (un) {
                un = un.toLowerCase();
                if ( ( un.indexOf('bot') > -1 ) || ( un == 'streamelements') || ( un == 'streamlabs') ) {
                  div.style.opacity = '.25';
                }
              }
            }
        ...
        m = null; um = null; un = null; u = null; divs = null; div = null; return false;
      }
...
function scbt_user_chat_text_only() {
  if (window.scbtChatWrapperRef.classList.contains('scbt-bl') ) {
      ...
      var divs =scbt_get_chat_messages();
      var toSearchFor = '.chat-message-identity';
      ...
      if (window.isdoTextOnlys === 0) {
        ...
        window.isdoTextOnlys = 1;
        divs = null; div = null; toSearchFor = null; x = null; imgs = null; is = null; i = null; return false;
      }
...
function scbt_user_chat_up_to_top() {
  if (window.scbtChatWrapperRef.classList.contains('scbt-bl') ) {
    ...
    var y = document.querySelectorAll('.overflow-y-scroll.py-3');
    if (y[0]) {
      y[0].scroll({top:0,behavior:'smooth'});
      y[0].animate({ scrollTop: 0 }, 100);
      y[0].scrollTop = 0;
    }
  }
...
function scbt_user_full_screen_height_chat() {
  if (window.isdoFullScreenHeight === 0) {
    var x = document.body.querySelectorAll('.flex.h-full.w-full.flex-col.overflow-hidden > .flex.grow.overflow-hidden');
    if (x[0]) {
      x[0].style.height = '100vh';
      x[0].style.overflow = 'visible';
      x[0].style.zIndex = '100';
    ...
    if (e.target) {
      if (e.target.dataset) {
        if (e.target.dataset.channelid) {
          ...
          if (e.target.dataset.service == 'kick') {
            name = name.replace(/ /g, '');
    ...
    store.getAll().onsuccess = function(e2) {
      ...
      if (searchType == 'byevent') {
        [].forEach.call(chats, function(item) {
          if (window.scbtUniqueMessageIds.includes(item.itemid) === false) {
            window.scbtUniqueMessageIds.push(item.itemid);
            if ( (item.timestamp == '1234') || (item.timestamp == '12345') || (item.timestamp == '123456') || (item.timestamp == '1234567') || (item.staff === 1) ) {
              var classString = scbt_get_classString_for_chat(item);
              if (item.timestamp == '1234567') {
                var m = item.message;
                ...
                var newElement = "<p class='" + classString + "'><span>" + "<span class='author-name'>" + item.username + "</span></span><b>" + imgElm + "</b></p>";
              } else {
                var newElement = "<p class='" + classString + "'><span>" + " : <span class='author-name'>" + item.username + "</span> </span><b>" + item.message + "</b></p>";
              }
              window.scbtChatContentRef.insertAdjacentHTML('beforeend', newElement);  
              }
            }
...
function scbt_user_theatre_mode() {
 var x = document.body.querySelectorAll('.chat-container');
 if (x[0]) {
 ...
 var y = document.body.querySelectorAll('.sidebar');
 if (y[0]) {
      ...
      if (window.scbtVisibility === 0) {
        ...
        [].forEach.call(divs, function(div) {
          div.style.opacity = 0;
            ...
            if (parameter == 'owner') {
              var z = scbt_get_role_from_message(div, 'owner'); // 0 or 1
              if (z > 0) {
                scbt_helper_chat_on(div);
                div.style.opacity = 1;
              }
            ...
            if (parameter == 'sub') {
              var z = scbt_get_role_from_message(div, 'sub'); // 0 or 1
              if (z > 0) {
                scbt_helper_chat_on(div);
                div.style.opacity = 1;
              }
/*
            ...
            if (parameter == 'moderator') {
              var z = scbt_get_role_from_message(div, 'moderator'); // 0 or 1
              if (z > 0) {
                scbt_helper_chat_on(div);
                div.style.opacity = 1;
              }
...
*/
            ...
            if (parameter == 'mod_sub') {
              var z = scbt_get_role_from_message(div, 'sub'); // 0 or 1
              var zz = scbt_get_role_from_message(div, 'moderator'); // 0 or 1
              if ( (z > 0) || (zz > 0)) {
                scbt_helper_chat_on(div);
                div.style.opacity = 1;
              }
...
*/
           ...
           if ( (parameter == 'vip') || (parameter == 'verified') ) {
              var z = scbt_get_role_from_message(div, 'verified'); // 0 or 1
              if (z > 0) {
                scbt_helper_chat_on(div);
                div.style.opacity = 1;
              }
...
function scbt_user_toggle_sub_event_chats() {
  if (window.scbtChatWrapperRef.classList.contains('scbtSearchOpen') ) {
      ...
      var divs = document.body.querySelectorAll('#chat-messages .bubble');
...
function scbt_user_go_to_stream_timestamp(e) {
    if (e) {
      e.preventDefault();
      if (e.target) {
        if (e.target.dataset) {
          if (e.target.dataset.minutesin) {
            var video = document.getElementsByTagName('video')[0];
            video.autoplay = true;
            // video.muted = true;
            // video.click();
            var seconds = Number(e.target.dataset.minutesin) * 60;
...
function scbt_start_setup() {
    if (window.scbtFontUp) {  } else {
      ...
      if ( window.hasOwnProperty('scbtIsKick') ) {  } else { window.scbtIsKick = true; }
      ...
      setTimeout(function() {
        if (window.scbtIsMobile === true) {
          window.scbtA = document.body.querySelector('.main-html');
          window.scbtB = document.body.querySelector('.h-full > nav');
          window.scbtC = document.body.querySelector('#chatroom > .relative');
        }
...
chrome.runtime.onMessage.addListener(
  function(request, sender, goCapture) {
    ...
    if (typeof request.message === 'object') {
      ...
      U = request.message.url;
      if ( (request.message.status == 'complete') && (request.message.active === true) && (U.indexOf('https://kick.com/video/') > -1) ) {
        console.log('This is a VOD');
        var videoIDArr = U.split('https://kick.com/video/');
        if (window.scbtisVOD === false) {
          window.scbtisVOD = true;
          setTimeout(function() {
            scbt_helper_load_chat_replay_from_api(videoIDArr[1]);
          }, 6000);
        }
      }

      // livestream
      if (U == window.scbtCurrentURL) { } else {
        // console.log('new page loaded ' + window.scbtCurrentURL + ' ' + U);
        window.scbtCurrentURL = U;
        scbt_make_toast();
    ...
    if (request.message === 'clicked' || request.message === 'kickstarrchatloaded') {
      scbt_start_setup();
    }

    if (window.scbtKeybindsOn === true) {
      if (request.message === 'kickstarrcommand1') {
        console.log('kickstarrcommand1 command');
        scbt_user_command1();
      }
