/* Copyright 2023 biolithic. All rights reserved. rumble
variable types:
...
function scbt_helper_csv_import_chat_log_from_chatarr(filename, chatArr) {
  ...
  var two = fileNamePartsArr[1];    // rumble
              ...
              if (mutation.addedNodes) {
                ...
                for (var i=0; i < arrl; i++) { 
                  ...
                  if (window.scbtOptions.scbtfeature8 === true) {
                    var elem = document.body.querySelector('#chat-history-list');
...
function scbt_get_str_channelid() {
  ...
  if (elem2Arr[0]) {
    channelElem = elem2Arr[0];
  }
  if (channelElem) {
    channelid = channelElem.textContent;
    if (channelid) {
      channelid = channelid.trim();
      channelid = channelid.replace(/ /g, '_');
...
function scbt_helper_build_all_menus() {
  ...
  if (elemArr[0]) { } else { 
    (async () => {
      await scbt_helper_get_menu(window.scbtXRef, 'scbtSideMenu', 'side_menu_rumble.html', false);
    })();
  }
  ...
  if (elemArr[0]) { } else {
     (async () => {
      ...
      scbt_helper_toast( window.scbtserviceid + ' Rambler loaded, RUNNING version: ' + chrome.runtime.getManifest().version);
      console.log(' Rambler loaded, RUNNING version: ' + chrome.runtime.getManifest().version);
  ...
  if (typeof obj == 'object') {
    ...
    if (obj.a == 'scbttheme1') {
      if (obj.b == '#ffffff' || obj.b == '') {
        css = css + '#chat-history-list { background-color: initial !important; } .scbtChatWrapper { background-color: initial; } ';
      }
      if (obj.b == '#000000') {
        css = css + '#chat-history-list { background-color: transparent !important; } .scbtChatWrapper { background-color: transparent; } ';
      }  
      if ( (obj.b != '#ffffff') && (obj.b != '#000000') ) {
        css = css + '#chat-history-list { background-color:' + obj.b + ' !important; } .scbtChatWrapper { background-color:' + obj.b + '; } '; 
      }
    }
    ...
    if (obj.a == 'scbttheme2') {
      if (obj.b == '#ffffff') {
        css = css + '.chat-history--username a { color: unset !important; } .scbt-chat-username { color: unset; } ';
      }
      if (obj.b == '#000000') {
        css = css + '.chat-history--username a { color: transparent !important; } .scbt-chat-username { color: transparent; } ';
      }
      if ( (obj.b != '#ffffff') && (obj.b != '#000000') ) {
        css = css + '.chat-history--username a { color:' + obj.b + ' !important; } .scbt-chat-username { color: ' + obj.b + ' ; } ';
      }
    }
    ...
    if (obj.a == 'scbttheme4') {
      if (obj.b == '#ffffff') {
        css = css + '.chat-history--message { color: unset !important; text-shadow: 0px; } .scbt-chat-message { color: unset; text-shadow: 0px; } ';
      }
      if (obj.b == '#000000') {
        css = css + '.chat-history--message { color: #111111 !important; text-shadow: 0.5px 0.5px silver; } .scbt-chat-message { color: #111111; text-shadow: 0.5px 0.5px silver; } ';
      }  
      if ( (obj.b != '#ffffff') && (obj.b != '#000000') ) {
        css = css + '.chat-history--message { color:' + obj.b + ' !important; text-shadow: 0.5px 0.5px darkslategrey; } .scbt-chat-message { color:' + obj.b + ' ; text-shadow: 0.5px 0.5px darkslategrey; } ';
      }
    }
    ...
    if (obj.a == 'scbttheme5') {
      if (obj.b > 0) {
        var t = obj.b + 'rem';
        css = css + ' .chat-history--message { font-size: ' + t + '; line-height: ' + obj.b  + '; } .scbt-chat-message { font-size: ' + t + '; line-height: ' + obj.b  + '; } ';
      } else {
        css = css + ' .chat-history--message { font-size: initial; line-height: unset; } .scbt-chat-message { font-size: initial; line-height: unset; } ';
      }
    }
    ...
    if (obj.a == 'scbttheme6') {
      if (obj.b === true) {
        css = css + ' .chat-history--row img, .chat-history--row svg, .chat-history--username, .chat-history--badges-wrapper { visibility: hidden !important; } .scbt-chat-username, .scbt-chat-timestamp { visibility: hidden; } ';
      } else {
        css = css + ' .chat-history--row img, .chat-history--row svg, .chat-history--username, .chat-history--badges-wrapper { visibility: visible !important; } .scbt-chat-username, .scbt-chat-timestamp { visibility: visible; } ';
      }      
    }
...
function scbt_helper_load_options(clicked) {
  ...
  if (
    ...
    h == 'https://rumble.com/subscriptions/' ||
    h == 'https://rumble.com/watch-history/' ||
    h == 'https://rumble.com/editor-picks/' ||
    (h.indexOf('https://rumble.com/account/') > -1)
  ) { return false; }
    ...
    if ( (window.hasOwnProperty('scbtIsRumble') === true) && (window.scbtIsRumble === true) ) { } else {
      ...
      if (elemArr[0]) { } else {
        ...
        window.scbt1ElementRef = document.body.getElementsByClassName('nonconstrained')[0];
        window.scbt2ElementRef = document.body.querySelectorAll('header.header')[0];

        if ( window.location.href.indexOf('/chat/popup/') > -1) {
          window.scbt1ElementRef = document.body.getElementsByClassName('nonconstrained')[0];
          window.scbt2ElementRef = document.body.querySelectorAll('section.chat')[0];
        }

        if (window.scbt1ElementRef || window.scbt2ElementRef) {
          scbt_helper_build_all_menus();
        } else {
          console.log('Error: stream/page not supported. Please refresh on a live stream.');
      ...
      // if this is a video page like https://rumble.com/v2xvkzm-abc.html
      ...
      // https://rumble.com/chat/popup/123
      if ( window.location.href.indexOf('/chat/popup/') > -1) {
        window.scbtchannelid = null;
        window.scbtvideoid = null;
        window.scbtDbNameToSearch = null;
        ...
        var elemArr = document.getElementsByClassName('chat--subscribe-header--title');
        if (elemArr[0]) {
          var t = elemArr[0].textContent;
          if (t) {
            t = t.trim();
            t = t.replace(/ /g, '_');
            t = t.replace(/\W/g, '');
            window.scbtchannelid = t;
          }
        }
        var arr = window.location.href.split('/chat/popup/');
        if (arr[1]) {
          window.scbtvideoid = arr[1];  
        }
        // scbt_get_str_dbname('startSaving');
        window.scbtDbName = 'savedchat' + '&' + window.scbtserviceid + '&' + window.scbtchannelid + '&' + window.scbtvideoid;
        window.scbtDbNameToSearch = window.scbtDbName;
...
chrome.runtime.onMessage.addListener(
    ...
    if (typeof request === 'object') {
      if (typeof request.tabUpdated === 'object') {
        if (request.tabUpdated.url) {
          if ( request.tabUpdated.url.indexOf('https://rumble.com/') > -1 ) {
  ...
  if (request.chatloaded && request.chatloaded == 'ramblerchatloaded') {
    scbt_helper_load_options('clicked');
  }

  if (window.scbtKeybindOnIs === true) {
    if (request.message === 'ramblercommand1') {
      scbt_user_command1();
    }
    if (request.message === 'ramblercommand2') {
      scbt_user_command2();
    }
    if (request.message === 'ramblercommand3') {
      scbt_user_command3();
    }
    if (request.message === 'ramblercommand4') {
      scbt_user_command4();
    }
  }
});